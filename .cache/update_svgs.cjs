...
let main_js = fs.readFileSync('src/main.js', 'utf8');
main_js = main_js.replace(/export function getVariantSvg\(id\) \{[\s\S]*?\n\}/, 
...
fs.writeFileSync('src/main.js', main_js, 'utf8');

let inject_js = fs.readFileSync('inject_svg.js', 'utf8');
inject_js = inject_js.replace(/function getVariantSvg\(id\) \{[\s\S]*?\n\}/, 
...
inject_js = inject_js.replace(/const map = \{[\s\S]*?\};/, new_map);
inject_js = inject_js.replace(/for \(let i = 1; i <= 26; i\+\+\)/, 'for (let i = 1; i <= 43; i++)');
fs.writeFileSync('inject_svg.js', inject_js, 'utf8');

console.log("Updated main.js and inject_svg.js successfully.");
