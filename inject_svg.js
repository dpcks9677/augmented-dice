...
const mdPath = '.documents/augments_explaination.md';
let content = fs.readFileSync(mdPath, 'utf8');

content = content.replace(/<svg[\s\S]*?<\/svg>\s*/g, '');

for (let i = 1; i <= 43; i++) {
  const svg = getVariantSvg(map[i]);
  const regex = new RegExp("\\*\\s+\\*\\*(" + i + "\\. )(.*?)\\*\\*");
  content = content.replace(regex, "*   **" + svg + " $1$2**");
}

fs.writeFileSync(mdPath, content, 'utf8');
console.log("Updated augments_explaination.md");
